...
class Player:
    ...
    def draw(self, display, scroll):  # отрисовка
        ...
        display.blit(pygame.transform.flip
                     (pygame.transform.rotate(image, self.rotation),
                      self.flipped, False), (self.player_rect.x - scroll[0],
                                             self.player_rect.y -
                                             scroll[1] - 2))
        mouse_x, mouse_y = pygame.mouse.get_pos()
        rel_x, rel_y = mouse_x - self.player_rect.x - scroll[0] + \
                       image.get_width() / 2, mouse_y - self.player_rect.y \
                       - scroll[1]
