...
from enemy import Enemy, FlyingEnemy, FlyingDestoryer
import sys
pygame.init()
...
maps = ["rcc/maps/level1.txt","rcc/maps/level2.txt", "rcc/maps/level3.txt", "rcc/maps/level4.txt"]
...
FPS_FONT = pygame.font.Font("rcc/font/AvenuePixel-Regular.ttf", 40)
SMALL_FONT = pygame.font.Font("rcc/font/AvenuePixel-Regular.ttf", 20)
...
while menu:
    ...
    main_menu_text = framework.render_fps_font(FPS_FONT, "FUG")
...
while not menu:
    ...
    for bullet in bullets:
        ...
        for tile in tile_rects: 
            if pygame.Rect(tile[0] - scroll[0], tile[1] - scroll[1], tile[2], tile[3]).colliderect(pygame.Rect(bullet.x - scroll[0], bullet.y - scroll[1], 4, 4)):
                ...
                framework.play_sound("rcc/sound_effects/explosion.wav")
                screen_shake = 10
                for x in range(50):
    ...
    if int(gold_count) >= gold_per_level[map_index]:
        if map_index != 3:
            ...
            FONT = pygame.font.Font("rcc/font/AvenuePixel-Regular.ttf", 35)
            text4 = framework.render_fps_font(FONT, "Вы прошли игру")
            display.blit(text4, (24, 100))
                    
    framework.handle_particles(display, scroll)

    if map_index == 0:
        text = framework.render_fps_font(SMALL_FONT, "Заполучите золото")
        display.blit(text, (150 - scroll[0], 100 - scroll[1]))

        text2 = framework.render_fps_font(SMALL_FONT, "Нажмите для атаки на тачпад")
    ...
    for g in gold:
        if pygame.Rect(player.player_rect.x - scroll[0], player.player_rect.y - scroll[1], player.player_rect.width, player.player_rect.height).colliderect(g[0] - scroll[0], g[1] - scroll[1], 16, 16):
            framework.play_sound("rcc/sound_effects/pickup.wav")
            gold.remove(g)
            for x in range(30):
    ...
    for bullet in enemy_bullets:
        ...
        player.player_rect.width, player.player_rect.height):
            if not dead:
                framework.play_sound("rcc/sound_effects/player_death.wav")
                screen_shake = 10
                dead = True
